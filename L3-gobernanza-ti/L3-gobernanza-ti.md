# L3 — Plan de Gobernanza TI

**Horizonte:** S4–S9 (Fases 1+2) · S10–S12 (Fase 3)  
**Esfuerzo estimado:** 82 hh  
**Cierre Fases 1+2:** ~29 Mayo 2026  
**Cierre Fase 3:** ~19 Junio 2026  
**Precondicion:** Catastro Level A requiere instrumento validado

---

## Descripcion

Framework documental y normativo que da cobertura legal y operativa al area TI. Sin politicas aprobadas, los hallazgos Deloitte no pueden cerrarse formalmente aunque esten resueltos tecnicamente.

---

## Categorias

| Categoria | Carpeta | Estado |
|---|---|---|
| Infraestructura digital del area | [[infraestructura-digital/L3-infraestructura-digital]] | Pendiente |
| Catastro de aplicaciones | [[catastro-aplicaciones/L3-catastro-aplicaciones]] | Pendiente |
| Diagnostico normativo | [[diagnostico-normativo/L3-diagnostico-normativo]] | Pendiente |
| Politicas y procedimientos | [[politicas-procedimientos/L3-politicas-procedimientos]] | Pendiente |
| Cierre y evidencia | [[cierre-evidencia/L3-cierre-evidencia]] | Pendiente |

---

## Dependencias externas

- Catastro Level A (G2) → habilita diagnostico normativo y WS-4
- Politicas firmadas (G3) → habilita cierre de hallazgos Deloitte
