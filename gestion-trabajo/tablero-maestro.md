...
| `workspace/dashboard` | Visualización local del vault + reporte mensual ejecutivo | definido | Implementar vista previa mensual y descarga Markdown sin duplicar lógica |
...
| [[gestion-trabajo/planes-activos/reporte-mensual-dashboard-workspace|Reporte mensual ejecutivo dashboard + convención workspace PM]] | listo-para-ejecutar | `workspace/dashboard` → `workspace/vault` | Implementar preview + descarga y luego actualizar guía/backlog del workspace |
...
- El dashboard necesita una salida ejecutiva enviable por correo para cierre mensual de actividades
...
Ejecutar en `workspace/dashboard` el plan `reporte-mensual-dashboard-y-convencion-workspace-pm` y dejar luego el handoff documental para `workspace/vault`.
